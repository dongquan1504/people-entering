...
import { Box, BoxProps, Tab, Tabs, Theme } from "@mui/material";
...
interface BasicTabsProps {
  ...
  tabSx?: BoxProps["sx"];
  className?: string;
  queryKey?: string; // for changing tab in url with location.hash
  noUsingQueryKey?: boolean;
...
export default function BasicTabs({
  ...
  const [selectedTab, setSelectedTab] = useState<any>(defaultTab);

  const handleChangeTabSearch = (value: number | string) => {
  ...
  const handleChange = (event: SyntheticEvent, newValue: number | string) => {
  ...
  useEffect(() => {
    ...
    setSelectedTab(queryValue);
    handleChangeTabSearch(queryValue as string);
  }, [queryValue]);
