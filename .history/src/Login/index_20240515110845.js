...
function Login() {
  ...
  const handleSubmit = () => {
    const user = fakeData.find(user => user.email === loginForm.email && user.password === loginForm.password);
    if (user) {
      navigate("/home");
    }
    setError(true)
  ...
  return (
    <div className="login">
      ...
      {isError&&<Box sx={{ color: "red", size: "small" }}>invalid email or password</Box>}
