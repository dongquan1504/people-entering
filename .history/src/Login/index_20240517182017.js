...
function Login() {
  ...
  const handleSubmit = () => {
    get(ref(db, "users"))
      .then((snapshot) => {
        if (snapshot.exists()) {
          const user = Object.values(snapshot.val()).find((user) => {
            const hash = "";
            console.log(user.password);
            bcrypt.hash(loginForm.password, 10, function async(err, newHash) {
              hash = newHash;
            });
            return user.email === loginForm.email && user.password === hash;
          });
